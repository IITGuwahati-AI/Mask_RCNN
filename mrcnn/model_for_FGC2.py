...
from mrcnn import utils_for_FGC as utils
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, gt_attr_ids, config):
    ...
    gt_masks = tf.gather(gt_masks, tf.where(non_zeros)[:, 0], axis=2,
                         name="trim_gt_masks")
    gt_attr_ids = tf.boolean_mask(gt_attr_ids, non_zeros, name="trim_gt_attr_ids")       ##Added by CA for FGC
    ...
    gt_attr_ids = tf.gather(gt_attr_ids, non_crowd_ix)                                  ## Added by CA for FGC
    ...
    roi_gt_attr_ids = tf.gather(gt_attr_ids, roi_gt_box_assignment)
    ...
    roi_gt_attr_ids = tf.pad(roi_gt_attr_ids, [(0, N + P), (0, 0)])   
    ## Added for FGC .. ---> This means that we are adding 
    # 0 rows on top of inout tensor, 
    # N+P rows at the bottom of intput tensor, 
    # 0 columns at left of input tensor 
    # and 0 columns at the right of input tensor.

    return rois, roi_gt_class_ids, deltas, masks, roi_gt_attr_ids
...
class DetectionTargetLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        gt_boxes = inputs[2]
        gt_masks = inputs[3]
        gt_attr_ids = inputs[4]

        # Slice the batch and run a graph for each slice
        # TODO: Rename target_bbox to target_deltas for clarity
        names = ["rois", "target_class_ids", "target_bbox", "target_mask", "target_attr_ids"]
        outputs = utils.batch_slice(
            [proposals, gt_class_ids, gt_boxes, gt_masks, gt_attr_ids],
            lambda w, x, y, z, a: detection_targets_graph(
                w, x, y, z, a, self.config),
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
                         pool_size, num_classes, num_attrs, train_bn=True,
                         fc_layers_size=1024):
    ...
    num_attrs : number of attributes.  for each predicted bbox and class ID we will predict set of attributes.      ## Added for FGC
    train_bn: Boolean. Train or freeze Batch Norm layers
    ...
    ## Attribute classifier head      ## Added for FGC
    mrcnn_attr_logits = KL.TimeDistributed(KL.Dense(num_attrs),
                                            name='mrcnn_attr_logits')(shared)
    mrcnn_attr_probs = KL.TimeDistributed(KL.Activation("sigmoid"),
                                     name="mrcnn_class_attributes")(mrcnn_attr_logits)
    

    return mrcnn_class_logits, mrcnn_probs, mrcnn_bbox, mrcnn_attr_probs
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            rois, target_class_ids, target_bbox, target_mask, target_attr_ids =\
                DetectionTargetLayer(config, name="proposal_targets")([
                    target_rois, input_gt_class_ids, gt_boxes, input_gt_masks, input_gt_attr_ids])

            # Network Heads
            # TODO: verify that this handles zero padded ROIs
            mrcnn_class_logits, mrcnn_class, mrcnn_bbox, mrcnn_attr_probs =\
                fpn_classifier_graph(rois, mrcnn_feature_maps, input_image_meta,
                                     config.POOL_SIZE, config.NUM_CLASSES, config.NUM_ATTR,
                                     train_bn=config.TRAIN_BN,
